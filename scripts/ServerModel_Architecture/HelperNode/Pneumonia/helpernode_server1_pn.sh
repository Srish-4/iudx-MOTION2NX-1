...
if [ ${layer_types[layer_id]} -eq 1 ] && [ $layer_id -eq 1 ];then 
   ...
   $build_path/bin/tensor_maxpool --my-id 1 --party 0,$cs0_host,$relu0_port_inference --party 1,$cs1_host,$relu1_port_inference --arithmetic-protocol beavy --boolean-protocol yao --fractional-bits $fractional_bits --filepath cnn_outputshare --current-path $build_path --strides 2 --pool-size 2
   #> $debug_1/tensor_gt_relu1_layer${layer_id}.txt &
   pid1=$!
   wait $pid1
   check_exit_statuses $?
