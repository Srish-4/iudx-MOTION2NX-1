...
if [ ${layer_types[layer_id]} -eq 1 ] && [ $layer_id -eq 1 ];then 
  ...
  # ####################################################################################
   ...
   $build_path/bin/tensor_maxpool --my-id 0 --party 0,$cs0_host,$relu0_port_inference --party 1,$cs1_host,$relu1_port_inference --arithmetic-protocol beavy --boolean-protocol yao --fractional-bits $fractional_bits --filepath cnn_outputshare --current-path $build_path --strides 2 --pool-size 2
   #> $debug_0/tensor_gt_relu0_layer${layer_id}.txt &
   pid1=$!
   wait $pid1
   check_exit_statuses $?
   ...
   tail -n +2 server0/outputshare_0 >> server0/cnn_outputshare_0
      ...
      $build_path/bin/sigmoid --my-id 0 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --arithmetic-protocol beavy --boolean-protocol yao --fractional-bits $fractional_bits --file-input outputshare_0
      pid1=$!
      wait $pid1
      check_exit_statuses $?
