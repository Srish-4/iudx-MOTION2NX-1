...
def call(image):
    ...
    try:
        ...
        canvas_1 = Canvas(window, width=1600, height=600)
        canvas_1.grid(row=1, column=0)

        # Load the original image
        original_image = Image.open(
            base_dir + "/data/ImageProvider/raw_images/" + image)
        # Resize the original image to fit the canvas
        resized_original_image = original_image.resize(
            (800, HEIGHT), Image.ANTIALIAS)
        original_photo_image = ImageTk.PhotoImage(resized_original_image)
        # Display the resized original image
        # center_x = 200
        # center_y = 200

        canvas_1.create_image(400, 0,
                              anchor=NW, image=original_photo_image)
