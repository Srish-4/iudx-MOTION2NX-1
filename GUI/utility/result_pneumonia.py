...
def call(image):
    ...
    try:
        ...
        canvas_2.create_text(850, 35, anchor=CENTER, text=application_title,
                             fill="black", font=('Roboto 50 normal'))

        canvas_1 = Canvas(window, width=WIDTH, height=HEIGHT)
        canvas_1.grid(row=1, column=0)

        back_image1 = PhotoImage(
            file=(base_dir+"/data/ImageProvider/raw_images/" + image))
        my_image2 = canvas_1.create_image(
            WIDTH/2, HEIGHT/2, anchor=CENTER, image=back_image1)

        # canvas = Canvas(window, width=WIDTH, height=HEIGHT)
        # canvas.grid(row=1, column=1)
        # img = cv2.imread(
        #     base_dir+"/data/ImageProvider/processed_images/" + image, cv2.IMREAD_UNCHANGED)
        ...
        Result_print = "Pneumonia status: " + str(result)
        canvas_3.create_text(850, 35, anchor=CENTER, text=Result_print,
                             fill="black", font=('Roboto 50 normal'))
        ...
        retry_button = Button(canvas_4, text="Run Again", padx=10, pady=10, command=lambda: retry(
            window), highlightthickness=3, highlightbackground="black", font=('Times 20 bold'))
        exit_button = Button(canvas_4, text="Exit", padx=10, pady=10, command=lambda: exit_app(
            window), highlightthickness=3, highlightbackground="black", font=('Times 20 bold'))
