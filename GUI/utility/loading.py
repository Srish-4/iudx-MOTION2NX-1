...
from utility import result_pneumonia
import subprocess
import os
global options
global option
    ...
    def background_task():
        ...
        while True:
            if (not temp):
                ...
                f = open("output.txt", "w")
                if options == 1:
                    iter = subprocess.call(
                        "python3 " + base_dir + "/Dataprovider/image_provider/MNIST_preprocess_image.py -f "+image, shell=True)
                # subprocess.call(base_dir + "/scripts/ServerModel_Architecture/Split/ImageProvider.sh", stdout=f)
                elif options == 2:
                    iter = subprocess.call(
                        "python3 " + base_dir + "/Dataprovider/image_provider/cifar_preprocess_image.py -f "+image, shell=True)
                elif options == 3:
                    iter = subprocess.call(
                        "python3 " + base_dir + "/Dataprovider/image_provider/pneumonia_preprocess_image.py -f "+image, shell=True)
                if option == 1:
                    # print(" option 1 is executed")
                    subprocess.call(
                        base_dir + "/scripts/ServerModel_Architecture/HelperNode/ImageProvider.sh", stdout=f)
                elif option == 2:
                    # print(" option 2 is executed")
                    # print("**************************************")
                    subprocess.call(
                        base_dir + "/scripts/ServerModel_Architecture/HelperNode/CNN/ImageProvider.sh", stdout=f)
                elif option == 3:
                    print(" option 3 is executed")
                    print("**************************************")
                    subprocess.call(
                        base_dir + "/scripts/ServerModel_Architecture/HelperNode/Pneumonia/ImageProvider.sh", stdout=f)
                else:
